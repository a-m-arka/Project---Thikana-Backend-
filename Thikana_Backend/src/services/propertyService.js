...
export const registerProperty = async (token, property, files) => {
  try {
    ...
    const addPropertyResponse = await propertyUtils.addNewProperty(
      user.user_id,
      property,
    );
    ...
    for (const image of uploadImageResponse.results) {
      const addImageResponse = await propertyUtils.addPropertyImage(
        propertyId,
        image.url,
        image.publicId,
      );
...
export const deleteProperty = async (token, propertyId) => {
  try {
    ...
    const ownershipResponse = await propertyUtils.checkUserPropertyOwnership(
      user.user_id,
      propertyId,
    );
    ...
    const deletePropertyResponse =
      await propertyUtils.deleteProperty(propertyId);
...
export const updatePropertyDetails = async (token, propertyId, newData) => {
  try {
    ...
    const updateResponse = await propertyUtils.updateProperty(
      propertyId,
      newData,
    );
...
export const deletePropertyImages = async (token, propertyId, imageIds) => {
  try {
    ...
    for (const imageId of imageIds) {
      const imageOwnershipResponse =
        await propertyUtils.checkPropertyImageOwnership(propertyId, imageId);
      ...
      if (!imageOwnershipResponse.ownership) {
        return {
          success: false,
          message: `This property doesn't have a image with id ${imageId}`,
        };
      }
    }
    ...
    for (const imageId of imageIds) {
      const deleteFromDbResponse =
        await propertyUtils.deletePropertyImage(imageId);
...
export const getUserProperties = async (token) => {
  try {
    ...
    const propertiesResponse = await propertyUtils.getAllPropertiesOfUser(
      user.user_id,
    );
...
export const getAllProperties = async () => {
  return propertyUtils.getAllProperties();
};

export const getPropertyById = async (propertyId) => {
  return propertyUtils.getPropertyById(propertyId);
};
